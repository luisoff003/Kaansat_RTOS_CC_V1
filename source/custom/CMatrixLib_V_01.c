...
*/
    ...
    for(k=0; k<N-1; k++)// k es el número de renglón / columna
    {
        for(j=k+1; j<N; j++)// j es el número de renglón
        {
            ...
            for(i=0; i<N; i++)// i es el número de columna
            {
...
*/
    ...
    for(j=N-1; j>=0; j--)// j es el número de renglón
    {
        for(i=0; i<N; i++)// i es el número de columna
        {
            sum=0.0;
            matrix_double_set(Mout, j+1, i+1, 0.0);
            for(k=j+1; k<N; k++) // k es el número de columna y renglón
            {

                sum=sum+matrix_double_get(Mi1, j+1, k+1)*matrix_double_get(Mout, k+1, i+1);

            }
            if(matrix_double_get(Mi1, j+1, j+1)==0.0)// j es el número de renglón y columna
            {
                return(0);
            }
            matrix_double_set(Mout, j+1, i+1, ((matrix_double_get(&baux, j+1, i+1)-sum)/matrix_double_get(Mi1, j+1, j+1)));// j es el número de renglón, i es de columna
        }
    }
